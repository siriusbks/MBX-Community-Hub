...
export default function MineboxV2Ending() {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [isVisible, setIsVisible] = useState(false);

    const { t } = useTranslation(["mineboxV2Ending", "common"]);

    useEffect(() => {
        const hasSeen = localStorage.getItem("mbx_mineboxV2Ending_seen");
        if (!hasSeen) {
            setIsVisible(true);
        }
    }, []);
    ...
    const handleClose = () => {
        localStorage.setItem("mbx_mineboxV2Ending_seen", "true");
        setIsVisible(false);
    };
