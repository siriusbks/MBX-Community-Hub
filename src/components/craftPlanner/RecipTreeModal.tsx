...
import { X, ChevronRight, ChevronDown, List, ClipboardCopy, Check, Loader2, Square, SquareAsterisk } from "lucide-react";
import { useTranslation } from "react-i18next";
import { useCraftItem } from "./useCraftItem";
import type { CraftIngredient } from "./craftPlannerTypes";
import { gatherResources, gatherCraftsOnly, scaleQuantityMap } from "./craftResourceUtils";
...
const TreeNode: React.FC<{
    ...
    return (
        <div>
            <div className="flex items-center gap-2 py-1 pr-1" style={{ paddingLeft: depth * 22 }}>
                {hasChildren ? (
                    <button
                        type="button"
                ...
                <span className="shrink-0 rounded bg-primary/20 px-2 py-0.5 text-center text-sm text-primary min-w-[3rem]">
                    x{displayedAmount.toLocaleString()}
                </span>
                <ItemImage itemId={ingredient.id} className="h-6 w-6 shrink-0 rounded [image-rendering:pixelated]" />
            ...
            {hasChildren && expanded && (
                <div>
                    {childIngredients!.map((child, index) => (
                        <TreeNode key={`${child.id}-${index}`} ingredient={child} multiplier={displayedAmount} depth={depth + 1} />
                    ))}
                </div>
            )}
        </div>
...
const RecipeTreeModal: React.FC<RecipeTreeModalProps> = ({ open, onClose, itemId, locale = "en" }) => {
    ...
    return createPortal(
        ...
        >
            <div className="relative flex h-[calc(100vh-2rem)] w-full max-w-[1200px] flex-col overflow-hidden rounded-lg border border-gray-700 bg-gray-900 text-white shadow-2xl">
                <div className="flex shrink-0 items-center justify-between border-b border-gray-700 bg-gray-800 px-4 py-3">
                    <h2 className="text-lg font-bold">
                        {t("craftPlanner.recipeTree.title")}
                    </h2>
                    ...
                    {!loading && !error && data && (
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex items-center gap-3 rounded-lg bg-gray-800/50 p-4">
                                <ItemImage itemId={data.id} className="h-10 w-10 shrink-0 rounded [image-rendering:pixelated]" />
                                <span className="flex-1 truncate text-base font-bold">{data.name}</span>
                                <label className="flex shrink-0 items-center gap-2 text-sm">
                                    ...
                                    <input
                                        ...
                                        className="w-20 rounded border border-gray-700 bg-gray-900 px-2 py-1 text-center text-white"
                                    />
                                </label>
                            </div>
                            ...
                            <div className="rounded-lg bg-gray-800/50 p-4">
                                <div className="mb-3 flex flex-wrap items-center gap-3">
                                    <List className="h-8 w-8 shrink-0 rounded bg-primary/10 p-1.5 text-primary" />
                                    ...
                                    <div className="ml-auto flex items-center gap-2">
                                        <button
                                            className="flex items-center gap-2 rounded bg-gray-600 px-2 py-1.5 text-sm text-white transition hover:bg-gray-500"
                                            onClick={() => setShowCrafts((v) => !v)}
                                        >
                                            {showCrafts ? <SquareAsterisk className="h-4 w-4" /> : <Square className="h-4 w-4" />}
                                        ...
                                        >
                                            {showBasic ? <SquareAsterisk className="h-4 w-4" /> : <Square className="h-4 w-4" />}
                                            {t("craftPlanner.basicResources.title")}
                                        </button>

                                        <button
                                            disabled={!showBasic && !showCrafts}
                                            className="flex items-center gap-2 rounded bg-gray-600 px-2 py-1.5 text-sm text-white transition hover:bg-gray-500 disabled:cursor-not-allowed disabled:opacity-40"
                                            onClick={onCopyCSV}
                                        >
