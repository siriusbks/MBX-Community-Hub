//import { CorsIfDev } from "./helper"; (temporary removed pending minebox fix)
// File used to do weird minebox job maths
// Have fun exploring this file ! (you won't have any fun)

export type XpJson = Record<
    string,
    { id: string; experience_per_level: number[] }
>;
...
export async function totalXpForLevel(
...
export async function progressForJob(
    ...
    const { level, xpIntoLevel, xpForNextLevel } = await levelFromTotalXp(
        job,
        totalXp
    );
    return {
