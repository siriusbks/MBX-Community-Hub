...
function getStatsEntries(
  ...
  return Object.entries(stats).reduce<Array<[string, [number, number]]>>(
    (acc, [key, value]) => {
      if (
        Array.isArray(value) &&
        value.length >= 2 &&
...
export function BestiaryCodexPage() {
  ...
  const [selectedCreature, setSelectedCreature] = useState<JsonObject | null>(
    null
  )
  ...
  useEffect(() => {
    ...
    if (selectedCreatureId) {
      params.set("id", selectedCreatureId)
    } else {
      params.delete("id")
    }
    ...
    async function fetchCreatureDetails() {
      ...
      try {
        ...
        if (!isJsonObject(data)) {
          throw new Error(
            "Bestiary details response is not a valid JSON object"
          )
        }
